### Code to create interaction plots for PJ cover predictors (climate only model)
...
load("R:/Shriver_Lab/PJcover/NoDensity.RData")
...
predict_fun <- function(coef=betaOut,heatload=heatload_mean,ppt=ppt_mean,tmax=tmax_mean,ppt_dev=ppt_dev_mean,tmax_dev=tmax_dev_mean,dens=dens_mean){
...
ggplot(data = Heatload_PPT, aes(x = heatload, y = median, color = ci.group)) +
  ...
  ylab("Predicted tree cover") + xlab("Heatload") +
  mytheme 
...
png(file="./Output/Interactions.png",9,3,units="in",type="cairo",res=600)
grid.newpage()
...
get.ind.heatppt.response<- function(j,sample){
  ...
  ppt_dev_pixel <- pixel_dat[,6] #seq(ppt_devrng_pixel[1], ppt_devrng_pixel[2], by = 0.1) #
  heatload_pixel <- mean(pixel_dat[,2])
  ppt_pixel <- mean(pixel_dat[,3])
  tmin_pixel <- mean(pixel_dat[,4])
  tmax_pixel <- mean(pixel_dat[,5])
  tmin_dev_pixel <- (pixel_dat[,7])
  tmax_dev_pixel <- (pixel_dat[,8])  
  dens_pixel <- latMean[1:36,subset] #mean(meanLatent) #
