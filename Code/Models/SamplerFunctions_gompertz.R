...
sampleSigma<-function(y,X,beta,a,b,eta,tmax,dd=F,ddint=F){  ###This function is based on Hooton and Heffly 
	##y=latent states
	...
	tmp.a<-a+n/2
	tmp.b<-b+(0.5*sum((y-X%*%beta-eta)^2))
...
sampleBeta<-function(y,X,beta.mn,beta.var,s2,eta,tmax,dd=F,ddint=F){  ###This function is based on Hooton and Heffly 
	##y=latent states
	##X=Xall including all interactions with density   
	X=apply(X,3,c)
	eta<-do.call(rbind, replicate((tmax-1), eta, simplify=FALSE))
	...
	tmp.mn<-tmp.var%*%(t(X)%*%(y-eta)/s2+Sig.beta.inv%*%beta.mn)
...
sampleAlpha2<-function(y,X,K,beta,alpha.mn, s2,t.Eta,dd=F,ddint=F){  ###This function is based on Hooton and Heffly 
	##y=latent states
	##X=Xall including all interactions with density   
	X=apply(X,3,c)
	if(dd==T|ddint==T){y=c(y[-1,])} else {y=c(y[-1,])-c(y[-tmax,])}
	K<-do.call(rbind, replicate((tmax-1), K, simplify=FALSE))
	hp=dim(K)[2]
	Sig.beta.inv=diag(hp)/t.Eta
	tmp.var<-solve(t(K)%*%K/s2+Sig.beta.inv)
	tmp.mn<-tmp.var%*%(t(K)%*%(y-X%*%beta)/s2+Sig.beta.inv%*%alpha.mn)
	
	return(as.vector(rmvnorm(1,tmp.mn,tmp.var,method='chol')))
	
}

sampleTauAlpha<-function(knots,alpha,aEta,bEta){  ###This function is based on Hooton and Heffly 
	##y=latent states
	##X=Xall including all interactions with density   
	
	
	tmp.a<-a+knots/2
	tmp.b<-b+(0.5*sum((alpha)^2))
	
	return(rinvgamma(1,tmp.a,tmp.b))
	
}

sampleLatent<-function(Y1,Y2,y,X,Xall,Xdense=NA,s2,beta,o1,o2,eta,Ndatasets,tmax,dd=F,ddint=F,ddbeta=NA,missing) {  #This function is tricky. I changes for first and last years and years for which we have one or two datasets
	...
	if(t>1 & dd==F & ddint==F){
		mu1<-y[t-1,]+((Xall[t-1,,]%*%beta) + eta) ####Here y is arranged year by pixel and X is year by pixel by covariate 
		Vi=Vi+1/s2
		v=v+mu1/s2
	}
	
	if(t>1 & (dd==T | ddint==T)){
		mu1<-(Xall[t-1,,]%*%beta) + eta ####Here y is arranged year by pixel and X is year by pixel by covariate 
		Vi=Vi+1/s2
		v=v+mu1/s2
	}
	
	if(t<tmax & dd==F & ddint==F){
		mu2<-y[t+1,]-((Xall[t,,]%*%beta) + eta) ####Here y is arranged year by pixel and X is year by pixel by covariate 
		Vi=Vi+1/s2
		v=v+mu2/s2
	}
	...
	if(t<tmax & dd==T & ddint==T){
		mu2<-(y[t+1,]-((X[t,,]%*%beta[-ddbeta]) + eta))/(beta[ddbeta[1]]+Xdense[t,,-1]%*%beta[ddbeta[-1]]) ####Here y is arranged year by pixel and X is year by pixel by covariate 
		Vi=Vi+1/s2
		v=v+mu2/s2
	}
...
load("Knot_cell_distances_subset.rda")
X <- X_lag
Xdense <- Xdense_lag
rm(X_lag,Xdense_lag)
...
#beta<-rep(0,dim(Xall)[3])+rnorm(dim(Xall)[3],0,0.2)
#beta_dd<-rep(0,dim(Xall_dd)[3])+rnorm(dim(Xall_dd)[3],0,0.2)
#beta_ddint<-rep(0,dim(Xall_ddint)[3])+rnorm(dim(Xall_ddint)[3],0,0.2)

eta<-matrix(0,length(K[,1]),1)
###Prior Parameters###
...
etaMean<-matrix(0,ncol(y),1)

alphaOut<-matrix(NA,iter,knots)
tauEtaOut<-matrix(NA,iter,1)
...
for (i in 1:iter){
	...
	beta<-sampleBeta(y,X=Xall,beta.mn,beta.var,s2,eta,tmax)
	betaOut[i,]<-beta
	
	s2<-sampleSigma(y,X=Xall,beta,a,b,eta,tmax)
	procVout[i,]<-s2
	...
	for (t in 1:tmax){
		...
		y[t,]<-sampleLatent(Y1,Y2,y,X=X,Xall=Xall,Xdense=Xdense,s2,beta,o1,o2,eta,Ndatasets,tmax,dd=F,ddint=F,ddbeta=NA,missing=missing_list)
	...
	alpha<-sampleAlpha2(y,X=Xall,K,beta,alpha.mn=rep(0,knots), s2,t.Eta)
	#alpha<-sampleAlpha(y,X=Xall,beta,K,s2,t.Eta,knots)
	alphaOut[i,]<-alpha
	
	eta<-K%*%alpha
	
	t.Eta<-sampleTauAlpha(knots,alpha,aEta,bEta)
	#t.Eta<-sampleTauEta(knots,alpha,aEta,bEta)
	tauEtaOut[i]<-t.Eta
	
	devOut[i] <- dev_calc(Y1,Y2,y,X=Xall,beta,o1,o2,s2,eta)
	
	if(i>burnin){
		latMean <- latMean*((i-(burnin+1))/(i-burnin)) + y*(1/(i-burnin))
		etaMean <- etaMean*((i-(burnin+1))/(i-burnin)) + eta*(1/(i-burnin))
	}   
...
for (i in 1:iter){
	...
	beta<-sampleBeta(y,X=Xall_dd,beta.mn_dd,beta.var,s2,eta,tmax,dd=T)
	betaOut_dd[i,]<-beta
	
	s2<-sampleSigma(y,X=Xall_dd,beta,a,b,eta,tmax,dd=T)
	procVout[i,]<-s2
	...
	for (t in 1:tmax){
		Vi=0
		v=0
		y[t,]<-sampleLatent(Y1,Y2,y,X=X,Xall=Xall_dd,Xdense=Xdense,s2,beta,o1,o2,eta,Ndatasets,tmax,dd=T,ddint=F,ddbeta=ddbeta,missing=missing_list)
	...
	alpha<-sampleAlpha2(y,X=Xall_dd,K,beta,alpha.mn=rep(0,knots), s2,t.Eta,dd=T)
	#alpha<-sampleAlpha(y,X=Xall,beta,K,s2,t.Eta,knots)
	alphaOut[i,]<-alpha
	...
	devOut[i] <- dev_calc(Y1,Y2,y,X=Xall_dd,beta,o1,o2,s2,eta,dd=T)
...
for (i in 1:iter){
	...
	beta<-sampleBeta(y,X=Xall_ddint,beta.mn_ddint,beta.var,s2,eta,tmax,dd=T,ddint=T)
	betaOut_ddint[i,]<-beta
	
	s2<-sampleSigma(y,X=Xall_ddint,beta,a,b,eta,tmax,dd=T,ddint=T)
	procVout[i,]<-s2
	...
	for (t in 1:tmax){
		...
		y[t,]<-sampleLatent(Y1,Y2,y,X=X,Xall=Xall_ddint,Xdense=Xdense,s2,beta,o1,o2,eta,Ndatasets,tmax,dd=T,ddint=T,ddbeta=ddbeta_int,missing=missing_list)
	...
	alpha<-sampleAlpha2(y,X=Xall_ddint,K,beta,alpha.mn=rep(0,knots), s2,t.Eta,dd=T,ddint=T)
	#alpha<-sampleAlpha(y,X=Xall,beta,K,s2,t.Eta,knots)
	alphaOut[i,]<-alpha
	...
	devOut[i] <- dev_calc(Y1,Y2,y,X=Xall_ddint,beta,o1,o2,s2,eta,dd=T,ddint=T)
