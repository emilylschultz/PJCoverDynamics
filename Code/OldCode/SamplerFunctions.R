...
sampleBeta<-function(y,X,beta.mn,beta.var,s2,tmax){  ###This function is based on Hooton and Heffly 
##y=latent states
...
sampleSigma<-function(y,X,beta,q,r,tmax){  ###This function is based on Hooton and Heffly 
  ##y=latent states
  ##X=Xall including all interactions with density   
  X=apply(X,3,c)
  y=c(y[-1,])-c(y[-tmax,])
...
sampleLatent<-function(Y1,Y2,y,X,Xall,Xdense=NA,s2,beta,o1,o2,Ndatasets,tmax,dd=F,ddint=F,ddbeta=NA) {  #This function is tricky. I changes for first and last years and years for which we have one or two datasets
  ...
  if(t<tmax & dd==T & ddint==F){
    mu2<-(y[t+1,]-X[t,,]%*%beta[-ddbeta])/(1+beta[ddbeta]) ####Here y is arranged year by pixel and X is year by pixel by covariate 
    Vi=Vi+1/s2
    v=v+mu2/s2
  }
  ...
  if(t<tmax & dd==T & ddint==T){
    mu2<-(y[t+1,]-X[t,,]%*%beta[-ddbeta])/(1+beta[ddbeta[1]]+Xdense[t,,]%*%beta[ddbeta[-1]]) ####Here y is arranged year by pixel and X is year by pixel by covariate 
    Vi=Vi+1/s2
    v=v+mu2/s2
  }
...
for (i in 1:iter){
  ...
  beta<-sampleBeta(y,X=Xall_dd,beta.mn_dd,beta.var,s2,tmax)
  betaOut_dd[i,]<-beta
  
  s2<-sampleSigma(y,X=Xall_dd,beta,q,r,tmax)
  procVout[i,]<-s2
...
for (i in 1:iter){
  ...
  beta<-sampleBeta(y,X=Xall_ddint,beta.mn_ddint,beta.var,s2,tmax)
  betaOut_ddint[i,]<-beta
  
  s2<-sampleSigma(y,X=Xall_ddint,beta,q,r,tmax)
  procVout[i,]<-s2
